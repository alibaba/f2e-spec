...
失败响应的响应体必须包含 `success` 字段，且值为 `false`，同时必须包含 `code` 字段，且值为数字或字符串，具体含义由业务决定，同时必须包含 `message` 字段，且值为字符串，具体内容由业务决定。

```json
{
  "success": false,
  "code": 10001, // 必须，错误码可为数字或字符串，具体含义由业务决定
  "message": "参数错误", // 必须，具体内容由业务决定
  "errors": [
    // 可选，如果包含了多个错误，则用此数组提供
    {
      "message": "名称不能为空", // 必须，具体内容由业务决定
      "field": "name" // 表单错误，可用此字段定位
    }
  ]
}
```
